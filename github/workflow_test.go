...
func TestFetchWorkflowRuns(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			targetRange: Range{
				Start: time.Date(2023, time.September, 1, 0, 0, 0, 0, time.UTC),
				End:   time.Date(2023, time.September, 30, 0, 0, 0, 0, time.UTC),
			},
			perPage:                  5,
			page:                     1,
			expectedTotalCount:       6,
			expectedWorkflowRunCount: 5,
		},
		{
